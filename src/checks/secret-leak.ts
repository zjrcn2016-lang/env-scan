...
import { SECRET_PATTERNS, KEYWORD_PATTERNS } from '../patterns';
import { isHighEntropy } from '../entropy';
import { readFileLines, isBinaryFile } from '../scanner';
 ...
 */
export function checkSecretLeak(sourceFiles: string[]): Finding[] {
  ...
  for (const filePath of sourceFiles) {
    ...
    for (let lineNum = 0; lineNum < lines.length; lineNum++) {
      ...
      for (const kw of KEYWORD_PATTERNS) {
        ...
        if (match) {
          ...
          const alreadyReported = findings.some(
            f => f.file === filePath && f.line === lineNum + 1
          );
      ...
      for (const sm of stringMatches) {
        const value = sm[1];
        const { isSecret, entropy } = isHighEntropy(value);
        if (isSecret) {
          const alreadyReported = findings.some(
            f => f.file === filePath && f.line === lineNum + 1
          );
 ...
 */
function maskSecret(value: string, fullMatch?: string): string {
 ...
 */
function limitFindingsPerFile(findings: Finding[], maxPerFile: number): Finding[] {
