...
```bash
npx @sako2026/env-scan                        # 在任意项目目录中运行
npm install -g env-scan              # 或全局安装
env-scan
```
...
```bash
env-scan [directory]                 # 扫描目录（默认：当前目录）
env-scan --secrets                   # 仅检查硬编码密钥
env-scan --git-exposed               # 仅检查 git 跟踪
env-scan --missing                   # 仅检查缺失变量
env-scan --unused                    # 仅检查未使用变量
env-scan --drift                     # 仅检查 .env/.env.example 漂移
env-scan --format json               # JSON 输出（用于 CI/CD）
env-scan --format markdown           # Markdown 报告
env-scan --severity high             # 仅显示高危及以上
env-scan --no-ignore                 # 不遵循 .gitignore 规则
env-scan --install-hook              # 安装 git pre-commit 钩子
env-scan --uninstall-hook            # 移除 git pre-commit 钩子
env-scan --strict                    # 将所有问题视为阻断性
```
...
```bash
npx @sako2026/env-scan --install-hook          # 安装
git commit -m "update"               # env-scan 自动先扫描
SKIP_ENVGUARD=1 git commit -m "..."  # 跳过检查
npx @sako2026/env-scan --install-hook --strict # 严格模式
```

该钩子在每次提交前运行 `env-scan --format json`，如果发现严重/高危问题则阻止提交。
...
```bash
npx @sako2026/env-scan                        # 一次性使用（无需安装）
npm install -g env-scan              # 全局安装
npm install --save-dev env-scan       # 本地开发依赖
```
